...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None, gridtype='hash', align_corners=False, interpolation='linear'):
        ...
        ''' 
            #* 根據最小的(base) resolution 16、最大的(desired) resolution 2048、level數量 num_levels=16
            #* 決定每個level 的resolution (base resolution * per_level_scale (1.38...) ** level)
        '''
        if desired_resolution is not None:
            per_level_scale = np.exp2(np.log2(desired_resolution / base_resolution) / (num_levels - 1))

        self.input_dim = input_dim #* 輸入的dim: 3 -> xyz
        self.num_levels = num_levels #* num levels, each level multiply resolution by 2
        self.level_dim = level_dim #* encode channels per level
        self.per_level_scale = per_level_scale #* multiply resolution by this scale at each level.
        self.log2_hashmap_size = log2_hashmap_size #* hashmap 大小, 19 -> 2^19
        self.base_resolution = base_resolution
        self.output_dim = num_levels * level_dim #* output dimension
        ...
        offsets = []
        offset = 0
        self.max_params = 2 ** log2_hashmap_size #* hash map 最大大小
        for i in range(num_levels): #* 16個level的resolution
            resolution = int(np.ceil(base_resolution * per_level_scale ** i)) #* 當前level的resolution
            params_in_level = min(self.max_params, (resolution) ** input_dim) #* hashmap大小, 最大2^19
            params_in_level = int(np.ceil(params_in_level / 8) * 8) # make divisible

            #* 應該是將hash map 疊起來, 紀錄每個hash map 的起始位置
        ...
        #* 所有hash map 儲存的參數，每個entry 儲存level_dim個參數
        self.n_params = offsets[-1] * level_dim

        # parameters
        #! nn.Parameters 作用是將 tensor 加入model.parameters中，這樣訓練時就能一起優化
